import type { Dictionary } from './dictionaries'

export const es: Dictionary = {
  app: {
    title: 'Compliance Tracker',
    subtitle: 'Panel de obligaciones de cumplimiento',
    description: 'Revisa obligaciones, vencimientos y estado de forma clara.',
  },
  actions: {
    newObligation: 'Nueva obligación',
  },
  create: {
    title: 'Crear obligación',
    description: 'Registra una nueva obligación de cumplimiento y deja que el backend determine su ciclo de vida.',
    backToDashboard: 'Volver al tablero',
    submit: 'Crear obligación',
    submitting: 'Creando...',
    fields: {
      type: 'Tipo',
      title: 'Título',
      description: 'Descripción',
      dueDate: 'Vencimiento',
      owner: 'Responsable',
      requiresDocument: 'Requiere documento',
      documentName: 'Nombre del documento',
      companyTaxId: 'Tax ID',
    },
    help: {
      type: 'Selecciona el tipo de obligación.',
      documentOptional: 'Déjalo vacío si todavía no hay documento adjunto.',
      documentRequired: 'Agrega el nombre del documento antes de guardar.',
      companyTaxId: 'Ingresa el Tax ID completo. Después de crearla solo se mostrará enmascarado.',
    },
    errors: {
      validation: 'Revisa los campos del formulario e inténtalo de nuevo.',
      network: 'No pudimos conectar con el servidor. Intenta de nuevo.',
      api: 'No se pudo crear la obligación.',
    },
  },
  edit: {
    title: 'Editar obligación',
    description: 'Actualiza los campos editables sin modificar el estado controlado por el backend.',
    backToDetail: 'Volver al detalle',
    submit: 'Guardar cambios',
    submitting: 'Guardando...',
    fields: {
      type: 'Tipo',
      title: 'Título',
      description: 'Descripción',
      dueDate: 'Vencimiento',
      owner: 'Responsable',
      requiresDocument: 'Requiere documento',
      documentName: 'Nombre del documento',
      companyTaxId: 'Reemplazo de Tax ID',
    },
    help: {
      type: 'Selecciona el tipo de obligación.',
      documentOptional: 'Déjalo vacío para limpiar el nombre del documento, o conserva el valor actual para mantenerlo.',
      documentRequired: 'Déjalo vacío para limpiar el nombre del documento.',
      companyTaxId: 'Déjalo vacío para mantener el Tax ID actual. Ingresa uno nuevo completo para reemplazarlo.',
    },
    errors: {
      validation: 'Revisa los campos del formulario e inténtalo de nuevo.',
      network: 'No pudimos conectar con el servidor. Intenta de nuevo.',
      api: 'No se pudo actualizar la obligación.',
    },
  },
  filters: {
    label: 'Filtrar por estado',
    all: 'Todas',
    pending: 'Pendiente',
    in_progress: 'En proceso',
    submitted: 'Presentada',
    done: 'Terminada',
  },
  kpis: {
    total: 'Total',
    pending: 'Pendientes',
    in_progress: 'En proceso',
    submitted: 'Presentadas',
    done: 'Terminadas',
    overdue: 'Vencidas',
    due_30_days: 'Vencen en 30 días',
  },
  fields: {
    title: 'Título',
    type: 'Tipo',
    owner: 'Responsable',
    due_date: 'Vencimiento',
    overdue: 'Vencida',
    noResults: 'No hay obligaciones para este filtro.',
    emptyState: 'Todavía no hay obligaciones.',
    dueSoon: 'Vencen en 30 días',
    documentName: 'Documento',
    companyTaxIdMasked: 'Tax ID',
    currentVersion: 'Versión actual',
  },
  statuses: {
    pending: 'Pendiente',
    in_progress: 'En proceso',
    submitted: 'Presentada',
    done: 'Terminada',
  },
  types: {
    annual_report: 'Reporte anual',
    franchise_tax: 'Franchise tax',
    boi_report: 'Reporte BOI',
    registered_agent_renewal: 'Renovación de agente registrado',
  },
  detail: {
    title: 'Detalle de obligación',
    description: 'Revisa el estado actual, el historial y las transiciones válidas.',
    backToDashboard: 'Volver al tablero',
    overview: 'Resumen',
    transitions: 'Transiciones disponibles',
    auditTrail: 'Historial de auditoría',
    auditTrailDescription: 'Cambios de estado ordenados cronológicamente y registrados por el backend.',
    noDocument: 'Sin documento',
    noAuditTrail: 'Todavía no hay historial de auditoría.',
    noTransitions: 'No hay transiciones disponibles.',
    loadingAudit: 'Historial',
    transitioning: 'Actualizando...',
    currentRecordChanged: 'Este registro cambió mientras trabajabas. Actualiza para ver la última versión.',
    invalidTransition: 'Esa transición no está permitida desde el estado actual.',
    documentRequired: 'Se necesita un documento antes de enviar esta obligación.',
    networkError: 'No pudimos conectar con el servidor. Intenta de nuevo.',
    genericError: 'No se pudo completar la transición.',
    recordNotFound: 'No se encontró la obligación.',
    editObligation: 'Editar',
    deleteObligation: 'Eliminar',
    deleteTitle: 'Eliminar obligación',
    deleteConfirmMessage: '¿Seguro que deseas eliminar “{obligationTitle}”?',
    deleteConfirmWarning: 'Esta acción no se puede deshacer.',
    confirmDelete: 'Eliminar definitivamente',
    cancelDelete: 'Cancelar',
    deleting: 'Eliminando...',
    reloadRecord: 'Actualizar registro',
    retry: 'Reintentar',
    transitionTo: 'Cambiar a',
    selectAvailableTransition: 'Selecciona una transición disponible.',
    noDocumentHint: 'No hay documento adjunto.',
    transitionDisabled: 'Transición deshabilitada',
    onTime: 'Al día',
  },
  reasons: {
    document_required: 'Documento requerido',
    invalid_transition: 'Transición inválida',
  },
}
